...
```{r}
model <- lm_basic(perfectionism ~ 1 + pf$speed, data= pf)
...
**Answer**:al the slopes are statistically significant
...
**Answer**:yes there is a difference betweeen normal and slow users
...
```{r}
model <- lm_basic(social_boldness ~ 1 + tension + sensitivity, data= pf)
reg_table(model, level = .95)
```


**Answer**:Interpret the significance and signs of the slopes in the previous model
for the two traits that you selected. Do their signs match what you would
have expected? yes they all have similar postiive scores but the R-squared is low so the model does not necessarily fit the variation correctly
...
```{r}
model <- lm_basic(social_boldness ~ 1 + tension + sensitivity + speed + gender + fct_lump(country, 5), data= pf)
reg_table(model, level = .95)
```

Does this change the slopes for your two traits much?

**Answer**: the slopes slightly decreased but no real big change.

Summarize the previous model (specifically the relationship between the
three traits) in words.

**Answer**: There is not a statisitcal significant difference between the countries becasue all of their confidence intervals contain zero except for the last one. all the traits hwoever are statistically significant
