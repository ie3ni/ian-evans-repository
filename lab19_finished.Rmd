...
**Answer**:burritos have a yelp score of 3.93 and the confidence interval does not contain zero and the entire interval is great than 3.7 so this is not just noise in the data. 
...
**Answer**: This data does not hold this assumption because the two who collected it went on yelp and found the better burrito shops to not waste their time. 
...
**Answer**: 3.91176 + .05699 is the slope coefficient of chips included (estimated average yelp score of restraunts who serve chips )
...
**Answer**: Nope because the confidence intervl contains zero
...
```{r}
burrito$synergy_new <- factor(burrito$synergy)
model <- lm_basic(overall ~ 1 + synergy_new, data = burrito )
reg_table(model, level= .95)
```
