...
Answer: There is a postive correlation. (An upward trend)
...
```{r}
ggplot(birds, aes( male_mass, egg_mass, alpha= 0.15)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10() +
  geom_point(color = "red", data = parrots) +
  annotate("rect", xmin= 40, xmax= 180, ymin= 3 , ymax= 12, fill = "purple" , alpha=.2) +
  annotate("text", x=100, y=10, label= "parrots", size=10) 
...
Answer:Yes the best-fit match shows a postive correlation or upward trend
...
ggplot(birds, aes( male_mass, egg_mass, alpha= 0.15)) +
  ...
  scale_x_log10() +
  scale_y_log10() +
  annotate("text", x=218.7, y=95, label= "Red-tailed tropicbird")
```
...
```{r}
ggplot(birds, aes(clutch_size, display)) +
  geom_point() +
  annotate("text", x=14.78, y=1.125, label= "Red-tailed tropicbird") +
  xlab("clutch size)") +
  ylab("display") +
  ggtitle("Clutch size vs. Display")
```
