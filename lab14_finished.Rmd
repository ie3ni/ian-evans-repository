...
```{r}
birds_everything <- group_summarize(birds, type)
birds_everything
```
...
```{r}
ggplot(birds_everything, aes( log(egg_mass_median), log(male_mass_median))) +
  geom_point()
```
...
```{r}
birds_types <- filter(birds_everything, n >= 5)
birds_types
```
...
ggplot(birds_everything, aes( log(egg_mass_median), log(male_mass_median))) +
  geom_point()+
  geom_point(color= "red", data=birds_types, aplha=7, size = 5) +
  geom_text(aes(label = "five or more birds"), data= birds_types, size = 5)
